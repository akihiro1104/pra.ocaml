...
(*ローマ字で駅名とリストを投げたら、漢字の名前を返す*)

let rec reomaji_ekimei_list romahi_ekimei list = match list with
    [] -> ""
    | { romaji = r; kanji = k;} :: rest -> if romahi_ekimei = r then k
                                                                else reomaji_ekimei_list romahi_ekimei rest 

let test1 = reomaji_ekimei_list "hakata" global_ekimei_list
let test2 = reomaji_ekimei_list "sumiyoshi" global_ekimei_list = "住吉"


(*漢字の駅名二つとリストを受け取ったら、２間の距離を返す*)
(*# use "metoro.ml" ;;*)

let rec get_wkikan_kyori name1 name2 list = match list with
    [] -> infinity
    | { kiten = k; shuten = s; kyori = r;} :: rest -> 
        if name1 = k && name2 = s then r
                                  else if name1 = s && name2 = k then r
                                                              else get_wkikan_kyori name1 name2 rest

let test1 = get_wkikan_kyori "博多" "札幌" global_ekikan_list = infinity
let test2 = get_wkikan_kyori "営団成増" "和光市" global_ekikan_list = 2.1
let test3 = get_wkikan_kyori "和光市" "営団成増" global_ekikan_list =2.1


(*繋がっていたら、a駅からb駅まではXXkmですと返す。繋がっていなかったらXXという駅は存在していませんと返す*)

let rec kyori_wo_hyouji name1 name2 list = match list with
    [] ->  name1 ^ "駅と" ^ name2 ^ "駅は、繋がっていません。"
        | { kiten = k; shuten = s; kyori = r;} :: rest -> 
            if name1 = k && name2 = s then name1 ^ "駅と" ^ name2 ^ "駅の距離は" ^ string_of_float (r) ^ "kmです。"
                                      else if name1 = s && name2 = k then name1 ^ "駅と" ^ name2 ^ "駅の距離は" ^ string_of_float (r) ^ "kmです。"
                                                                     else kyori_wo_hyouji name1 name2 rest
