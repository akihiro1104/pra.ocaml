...
(*ひらがな順に整列後重複しているものを削除*)

let rec hojo_shokika lst ekimei0 = match lst with 
    [] -> [ekimei0] 
  | ({kanji = k; kana = a; romaji = r; shozoku = s} as ekimei) :: rest -> 
         match ekimei0 with {kanji = k0; kana = a0; romaji = r0; shozoku = s0} -> 
	        if a = a0 then hojo_shokika rest ekimei0 
	                  else if a < a0 then ekimei :: hojo_shokika rest ekimei0 
	                                 else ekimei0 :: lst 

(* テスト *) 
let test1 = hojo_shokika [] {kanji="茗荷谷"; kana="みょうがだに"; romaji="myogadani"; shozoku="丸ノ内線"} 
	    = [{kanji="茗荷谷"; kana="みょうがだに"; romaji="myogadani"; shozoku="丸ノ内線"}] 

let test2 = hojo_shokika [ 
	{kanji="池袋"; kana="いけぶくろ"; romaji="ikebukuro"; shozoku="丸ノ内線"}; 
	{kanji="御茶ノ水"; kana="おちゃのみず"; romaji="ochanomizu"; shozoku="丸ノ内線"}; 
	{kanji="新大塚"; kana="しんおおつか"; romaji="shinotsuka"; shozoku="丸ノ内線"}; 
	{kanji="本郷三丁目"; kana="ほんごうさんちょうめ"; romaji="hongosanchome"; shozoku="丸ノ内線"}; 
...
let rec seiretsu list = match list with
    [] -> []
    | first :: rest -> hojo_shokika (seiretsu rest) first
